...
import com.chouc.flink.lagou.config.ConfContants;
 ...
 * @date 2020/11/12
 */
public class FlinkKafkaWindowsMysqlSinkDemo {
...
//                        super.open(parameters);
//                        // 获取连接
//                        Class.forName(ConfContants.MYSQL_DRIVER);
//                        connection = DriverManager.getConnection(ConfContants.MYSQL_URL, ConfContants.MYSQL_USER, ConfContants.MYSQL_PASSWD);
//                        connection.setAutoCommit(false);
//                        ps = connection.prepareStatement("replace into tb_pv_uv (date_str,type,value,update_time) values (?,?,?,now())");
//                    }
//
//                    @Override
...
//                });
                ...
                .addSink(JdbcSink.sink("insert into tb_pv_uv (date_str,type,value,update_time) values (?,?,?,now())",
                (preparedStatement,stringStringIntegerTuple3) -> {
                    preparedStatement.setString(1,stringStringIntegerTuple3.f0);
                    preparedStatement.setString(2,stringStringIntegerTuple3.f1);
                    preparedStatement.setInt(3,stringStringIntegerTuple3.f2);
                }, JdbcExecutionOptions.builder().withBatchIntervalMs(5000).build()
                , new JdbcConnectionOptions.JdbcConnectionOptionsBuilder()
                .withDriverName(ConfContants.MYSQL_DRIVER)
                .withUrl(ConfContants.MYSQL_URL)
                .withUsername(ConfContants.MYSQL_USER)
                .withPassword(ConfContants.MYSQL_PASSWD)
                .build()));
