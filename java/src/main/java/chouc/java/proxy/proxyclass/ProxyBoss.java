package chouc.java.proxy.proxyclass;
...
public class ProxyBoss {
	...
	public static <T> T getMyProxy(Class<?> interfaceClass,Class<?> implementClass,float discout){
		return (T)Proxy.newProxyInstance(interfaceClass.getClassLoader(), new Class[] { interfaceClass }, new InvocationHandler() {
			@Override
			public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
				float originalValue = (float) method.invoke(implementClass.newInstance(),args);
				return originalValue*discout;
			}
		});
	}
}
